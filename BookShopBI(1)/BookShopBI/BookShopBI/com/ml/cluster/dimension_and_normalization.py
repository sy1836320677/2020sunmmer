...
def prepare_normaliz(start_time, end_time):
    ...
    hql_1 = "insert overwrite table user_dimension select t1.userId,t1.avg,t2.ordercount,t3.sessioncount from " \
            "(select userId,avg(subTotal) avg from bx_orders group by userId) t1 " \
         "join (select userId,count(orderId) ordercount from bx_orders group by userId) t2 on t1.userId=t2.userId " \
